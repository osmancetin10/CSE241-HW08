...
public class ArrayList<Type> implements List<Type> {
	...
	public ArrayList(ArrayList a) { //constructor that takes array
	...
	public void addAll(Collection<Type> object) {
		...
		for(i=0;i<((ArrayList) object).size();i++) {
			element = (Type) ((ArrayList) object).get_element(i);
			this.add(element);
		}
	}
	...
	public boolean containsAll(Collection<Type> object) {
		...
		if(control==((ArrayList) object).size()) return true;
		return false;
	}
	...
	public void retainAll(Collection<Type> object) {
		...
		for(i=0;i<this.size();i++) {
			...
			for(j=0;j<object.size();i++) {
				if(this.arr[i]==(Type) ((ArrayList) object).get_element(j)) control++;
			}
